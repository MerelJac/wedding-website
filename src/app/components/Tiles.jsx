    ...
    <div className="max-w-5xl mx-auto px-4 pb-16">
      ...
      >
        {faqItems.map((item, index) => (
          <div
            key={index}
            className="relative perspective h-48"
            onClick={() => handleFlip(index)}
            onMouseEnter={() => window.innerWidth > 768 && handleFlip(index)}
            onMouseLeave={() => window.innerWidth > 768 && handleFlip(index)}
          >
            <div
              className={`flip-card-inner w-full h-full shadow-lg cursor-pointer ${
                flippedIndex === index ? "rotate-y-180" : ""
              }`}
            >
              {/* Front */}
              <div className="flip-card-front bg-beige text-white flex items-center justify-center p-4 text-center text-xl font-bold bg-opacity-60">
                {item.question}
              </div>
              {/* Back */}
              <div className="flip-card-back bg-darkBeige text-white flex items-center justify-center p-4 text-center text-lg bg-opacity-60">
                {item.answer}
              </div>
            </div>
          </div>
